...
CBO_UNITS = {
    ...
    "w_growth": "Percent",
    "L_growth": "Percent",
    "I_total": r"Billions of FJ$",
    "L": "2012=100",
    "C": r"Billions of FJ$",
    "agg_pension_outlays": r"Billions of FJ$",
    "G": r"Billions of FJ$",
    "iit_revenue": r"Billions of FJ$",
    "payroll_tax_revenue": r"Billions of FJ$",
    "business_tax_revenue": r"Billions of FJ$",
    "wL": r"Billions of FJ$",
    "D": r"Billions of FJ$",
}
...
"""
Concordances mapping a SAM's own commodity and activity codes to the model's
consumption goods (I) and production industries (M).

EMPTY UNTIL A FIJI SAM IS SOURCED. OG-FJI runs single-industry (M=1, I=1)
today and needs neither dictionary; they are read only by
``ogfji.input_output`` when M > 1 or I > 1. Populate them with the codes from
whichever SAM or supply-and-use table is adopted -- see the note on
``SAM_FILENAME`` in ``ogfji/input_output.py`` for the sourcing order. The codes
are specific to the SAM, so do NOT copy a sibling country's lists.

Rules that prevent a degenerate multi-industry calibration:

* Assert the concordances PARTITION the SAM before anything else -- every
  activity in exactly one PROD_DICT industry, every commodity in exactly one
  CONS_DICT good. A one-line set-equality check catches whole classes of
  silent error: a mistyped code simply drops that account from every
  aggregation, and nothing complains.
* Put manufacturing (the capital-goods producer) LAST in PROD_DICT. OG-Core
  treats the last industry as the numeraire and the sole non-consumption
  producer: all investment, government, net-outflow and remittance demand
  loads on it, so its nominal output share is inflated and is never comparable
  to a value-added share.
* Never let real estate or dwellings stand alone as an industry. Its measured
  capital share is dominated by imputed owner-occupier rent, which is booked
  as operating surplus but is not corporate profit -- it pushes that industry's
  gamma toward 1 (degenerate in the CES production function) and dilutes its
  effective CIT. Fold it into a broader finance and business-services
  aggregate. The same caution applies to any activity whose surplus is
  dominated by imputed or resource rent.
* For Fiji specifically, expect tourism-related activities (accommodation,
  food service, transport) to carry weight that a generic grouping would
  scatter across several industries; group them deliberately rather than by
  inherited habit.
"""
CONS_DICT = {}
