...
Runs the OG-FJI ``Calibration`` with ``update_from_api=True`` and writes the
overlay back into ``ogfji_default_parameters.json`` so an offline run
...
Usage::
    ...
    uv run python -m ogfji.update_baseline
"""
...
JSON_PATH = os.path.join(
    os.path.dirname(__file__), "ogfji_default_parameters.json"
)
