"""
Macro parameters for the OG-FJI model.

The packaged default-parameter JSON is the source of truth for OG-FJI's macro
block: every macro parameter is a documented, point-in-time Fijian value held
there. Only a parameter whose documented source is genuinely a live API is
refreshed in this module when ``update_from_api=True``; everything else stays
in the JSON so a live pull cannot clobber a value sourced elsewhere.

This is a deliberate design choice inherited from the sibling repos, not an
oversight. Broad live pulls (IMF GFS, World Bank external debt) have repeatedly
overwritten hand-sourced country values with series that are missing or zero
for a small economy -- in OG-PHL an IMF GFS social-benefit series of zero set
``alpha_T=0`` and broke the steady-state solve outright. Fiji, as a small
Pacific island economy, is more exposed to that failure mode than most, so keep
...
def get_macro_params(
    ...
    """
    Return macro-parameter overrides for the OG-FJI calibration.

    Only ``g_y_annual`` is refreshed from a live API -- World Bank
    GDP-per-capita growth over the pre-pandemic window, its documented source.
    Every other macro parameter is held in the packaged JSON and is NOT pulled
    here, so a live update cannot clobber a value sourced elsewhere.

    The macro parameters and the Fijian institution that owns each number
    (search by role, and prefer the national source over an international
    re-publication of it):

      * alpha_T, alpha_G           -- Ministry of Finance budget estimates and
                                      the annual Economic and Fiscal Update
      * initial_debt_ratio,        -- Ministry of Finance / debt-management
        initial_foreign_debt_ratio,   reporting; cross-check the IMF Article IV
        zeta_D                        debt-sustainability analysis
      * gamma                      -- capital share = 1 - labour share; ILOSTAT
                                      or Fiji Bureau of Statistics national
                                      accounts. Fiji has a large subsistence
                                      and informal sector, so consider the
                                      Gollin self-employment adjustment rather
                                      than the raw labour share
      * zeta_K, world_int_rate     -- Chinn-Ito openness for zeta_K; Reserve
                                      Bank of Fiji and the Article IV for the
                                      sovereign spread
      * r_gov_shift, r_gov_scale   -- keep the Li et al. (2021) slope, but
                                      re-anchor the shift to Fiji's actual real
                                      effective rate on debt (debt service /
