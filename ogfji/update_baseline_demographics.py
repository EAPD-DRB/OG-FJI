...
``ogfji_default_parameters.json`` under the installed ogcore.
...
``get_pop_objs`` that ``ogfji.calibrate`` uses (UN data, with the cached
...
parameters are documented constants (see ``ogfji.macro_params`` and the
calibration docs), NOT pulled from live APIs, so they are deliberately not
touched here.

    python -m ogfji.update_baseline_demographics
"""
...
def regenerate():
    ...
    installed ogcore, matching ogfji.calibrate's get_pop_objs calls."""
    json_path = files("ogfji").joinpath("ogfji_default_parameters.json")
